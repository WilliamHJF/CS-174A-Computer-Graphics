...
export class Inertia_Demo extends Simulation {
    ...
    update_state(dt) {
        ...
        while (this.bodies.length < 100)
            this.bodies.push(new Body(this.data.random_shape(), this.random_color(), vec3(1, 1 + Math.random(), 1))
                .emplace(Mat4.translation(...vec3(0, 20, 0).randomized(10)),
                    vec3(0, 2, 0).randomized(2).normalized().times(3), Math.random()));

        for (let b of this.bodies) {
            // Gravity on Earth, where 1 unit in world space = 1 meter:
            b.linear_velocity[1] += dt * -9.8 * 0.1;
    ...
    display(context, program_state) {
        ...
        if (!context.scratchpad.controls) {
            ...
            program_state.set_camera(Mat4.translation(-0.75, -6.39, -89.53));    // Locate the camera here (inverted matrix).
        }
